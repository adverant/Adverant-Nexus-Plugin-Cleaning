/**
 * Usage Tracking Middleware for Cleaning Service
 *
...
function detectOperation(req: FastifyRequest): string {
  ...
  // Cleaning Service API operations
  if (path.includes('/tasks')) {
    if (method === 'POST') return 'task_create';
    if (method === 'PUT' || method === 'PATCH') return 'task_update';
    if (method === 'DELETE') return 'task_delete';
    if (path.includes('/assign')) return 'task_assign';
    if (path.includes('/complete')) return 'task_complete';
    if (path.includes('/verify')) return 'task_verify';
    return 'task_query';
  }
  if (path.includes('/cleaners')) {
    if (method === 'POST') return 'cleaner_create';
    if (method === 'PUT' || method === 'PATCH') return 'cleaner_update';
    if (path.includes('/availability')) return 'cleaner_availability';
    if (path.includes('/performance')) return 'cleaner_performance';
    return 'cleaner_query';
  }
  if (path.includes('/schedule')) {
    if (path.includes('/optimize')) return 'schedule_optimize';
    if (path.includes('/generate')) return 'schedule_generate';
    return 'schedule_operation';
  }
  if (path.includes('/inventory')) return 'inventory_operation';
  if (path.includes('/checklist')) return 'checklist_operation';
  if (path.includes('/inspection')) return 'inspection_operation';
